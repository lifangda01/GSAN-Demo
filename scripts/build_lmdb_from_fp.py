...
sys.path.append('.')
from stainaire.utils.lmdb import construct_file_path, check_and_add, build_lmdb  # noqa: E402
    ...
    for sequence in tqdm(filenames):
        for filename in copy.deepcopy(filenames[sequence]):
            ...
            filepath = construct_file_path(
                args.data_root, '.', sequence, filename,
                extension)
...
def get_all_fp():
    root = '/mnt/cloudNAS3/fangda/HnE/TCGA/balanced_40x_512/'
    seq = 'images'
    all_filenames = {seq: []}
    for wsi in tqdm(os.listdir(os.path.join(root, seq))):
        fl = [os.path.join(wsi, f) for f in os.listdir(os.path.join(root, seq, wsi)) if f.endswith('.png')]
        all_filenames[seq] += fl
    with open(os.path.join(root, "all_filenames.json"), "w") as outfile:
        json.dump(all_filenames, outfile)
