...
from stainaire.utils.distributed import is_master, master_only
from stainaire.utils.distributed import master_only_print as print
from stainaire.utils.io import save_pilimage_in_jpeg
from stainaire.utils.meters import Meter, add_hparams
from stainaire.utils.misc import to_cuda, to_device, requires_grad
from stainaire.utils.model_average import (calibrate_batch_norm_momentum,
                                           reset_batch_norm)
from stainaire.utils.visualization import tensor2pilimage
...
class BaseTrainer(object):
    ...
    def save_image(self, path, data):
        ...
        if is_master() and vis_images is not None:
            ...
            # FDL: all inputs are 0-to-1 by default
            # if vis_images.min() < 0:
            ...
            if self.cfg.trainer.image_to_tensorboard:
                self.image_meter.write_image(
                    image_grid, self.current_iteration)
    ...
    def test(self, data_loader, output_dir, inference_args):
        ...
        for it, data in enumerate(tqdm(data_loader)):
            ...
            for output_image, file_name in zip(output_images, file_names):
                ...
                # FDL: all images 0-to-1 by default
                output_image = tensor2pilimage(output_image.clamp_(0, 1),
