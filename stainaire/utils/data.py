...
class Augmentor(object):
    ...
    def _build_augmentation_ops(self):
        ...
        for key, value in self.aug_list.items():
            if key == 'resize_smallest_side':
                ...
                h, w = value.split(',')
                h, w = int(h), int(w)
                augs.append(alb.Resize(h, w, always_apply=1, p=1))
                ...
                self.resize_h, self.resize_w = h, w
            elif key == 'pad_if_needed':
                divisible_by = value
                augs.append(alb.PadIfNeeded(
                    min_height=None, min_width=None,
                    pad_height_divisor=divisible_by, pad_width_divisor=divisible_by
                ))
            elif key == 'rotate':
                ...
                # was applied in order to correctly modify keypoint data.
                if value:
                    augs.append(alb.VerticalFlip(always_apply=False, p=0.5))
            ...
            # FDL:
                ...
                br, ct = value.split(',')
                br, ct = float(br), float(ct)
                augs.append(alb.RandomBrightnessContrast(
                    brightness_limit=br, contrast_limit=ct,
                    always_apply=False, p=0.5))
            elif key == 'hue_saturation_value':
                h, s, v = value.split(',')
                h, s, v = int(h), int(s), int(v)
                augs.append(alb.HueSaturationValue(
                    hue_shift_limit=h, sat_shift_limit=s,
                    val_shift_limit=v,
                    always_apply=False, p=0.5))
            elif key == 'gaussian_blur':
                augs.append(alb.GaussianBlur(
                    sigma_limit=value, always_apply=False, p=0.1))
            elif key == 'gauss_noise':
                augs.append(alb.GaussNoise(
                    var_limit=value, always_apply=False, p=0.5))
            elif key == 'elastic_transform':
                alpha, sigma = value.split(',')
                alpha, sigma = int(alpha), int(sigma)
                augs.append(alb.ElasticTransform(
                    alpha=1, sigma=sigma, alpha_affine=alpha, always_apply=False, p=0.5))
            elif key == 'random_upscale':
                augs.append(alb.RandomScale(
                    scale_limit=(1.0, float(value)), always_apply=False, p=0.5))
            else:
    ...
    def _perform_paired_augmentation(self, inputs):
        ...
        #     self.augment_ops
        # FDL: no resize
        aug_ops_with_resize = self.augment_ops
