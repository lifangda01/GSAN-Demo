...
from stainaire.optimizers import Fromage, Madam
from stainaire.utils.distributed import get_rank
from stainaire.utils.distributed import master_only_print as print
from stainaire.utils.init_weight import weights_init
from stainaire.utils.model_average import ModelAverage
...
def get_scheduler(cfg_opt, opt):
    ...
    if cfg_opt.lr_policy.type == 'step':
        ...
        scheduler = lr_scheduler.MultiStepLR(
            opt,
            milestones=cfg_opt.lr_policy.milestones,
            gamma=cfg_opt.lr_policy.gamma)
    elif cfg_opt.lr_policy.type == 'constant':
        scheduler = lr_scheduler.LambdaLR(opt, lambda x: 1)
    elif cfg_opt.lr_policy.type == 'lambda':
        def lambda_rule(step):
            lr_l = 1.0 - max(0, step - cfg_opt.lr_policy.n_steps_before_decay) / float(
                cfg_opt.lr_policy.n_steps - cfg_opt.lr_policy.n_steps_before_decay + 1)
            return lr_l
...
def get_optimizer_for_params(cfg_opt, params):
    ...
    if hasattr(cfg_opt, 'larc'):
        opt = LARC(opt, 
                   trust_coefficient=cfg_opt.larc.trust_coefficient,
                   eps=cfg_opt.larc.eps,
                   clip=cfg_opt.larc.clip)
        print("Wrapped optimizer with LARC!!!")
    return opt
